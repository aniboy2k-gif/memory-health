...
**해당 여부 확인**: `~/.claude/hooks/memory-line-check.sh`가 존재하고 MEMORY.md가 경고(≥160줄 또는 ≥20,000문자)를 발생시키고 있다면 이 스킬의 대상이다.

> ★ **CSR #1825 (2026-08-10)**: 플랫폼 캡은 **200줄 OR 25,000문자(UTF-16)** 이며 **바이트가 아니다**.
> 행동 프로브: 24,899자 로드 / 25,099자 절단 · 10,734자가 **31,534바이트**여도 전문 로드.
> 한글은 바이트만 부풀릴 뿐 캡에는 오히려 여유를 준다. `wc -c` 판정 금지.
> 재검증은 `tests/cap-boundary-probe.sh`(행동 프로브)로 — `strings` 재실측은 판별력이 없다.
...
| MEMORY.md 안전 목표 | 안전 마진 | 140줄 / 17,500문자 | 캡 × 0.7 (우리 선택 — 플랫폼 보증 아님) |
...
| 최적화기 | `--fix` | **있음** | MEMORY.md를 ≤ 140줄 / ≤ 17,500문자로 압축 (승인 게이트 1회) |
...
1. **진단** — 현재 줄 수·**문자 수**(UTF-16) 측정. 바이트는 진단 표시용이며 판정에 쓰지 않는다
...
7. **검증** — 줄 ≤ 140 · 문자 ≤ 17,500 검증; `skill-audit.log`에 결과 기록

**완료 기준**: 줄 ≤ 140 **그리고** 문자 ≤ 17,500 + 감사 로그 기록 (1–7단계 모두 완료 시).
...
# 외부 의존 파일 (이 저장소 외부):
~/.claude/hooks/
├── memory-line-check.sh              # 구현: >= 160줄 또는 >= 20,000문자 시 경고
